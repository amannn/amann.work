...
In my case I'm reading `scrollTop` on every animation frame, feed that into a stream which all animatable elements subscribe to and transform it as necessary. Reading `scrollTop` via `requestAnimationFrame` produces better results in comparison to a scroll listener, as the latter can fire too often which will cause jank. Therefore every frame starts with a single read operation from the DOM followed by a few write operations. That avoids [layout thrashing](https://developers.google.com/web/fundamentals/performance/rendering/avoid-large-complex-layouts-and-layout-thrashing) and reduces CPU work.
...
To me, the most promising aspect of Hooks was having another tool in the box to create abstractions for shared behaviour. This can lead to [slimmer components](https://twitter.com/jamannnnnn/status/1057025220362543116). Also `useEffect` is just amazing in comparison to `componentDidMount`, `-DidUpdate` and `-WillUnmount`.
